...
const UpdateProgram = ({ program, onClose, onUpdate }: UpdateProgramProps) => {
  const [image, setImage] = useState<string>(program.image);
  const [name, setName] = useState(program.name);
  const [description, setDescription] = useState(program.description);
  const [showConfirm, setShowConfirm] = useState(false);

  // Reset form if program changes
  useEffect(() => {
  ...
  const handleConfirm = () => {
    onUpdate({
      ...
      name,
      description,
      dateTime: new Date().toLocaleString(), // optionally update timestamp
    });
