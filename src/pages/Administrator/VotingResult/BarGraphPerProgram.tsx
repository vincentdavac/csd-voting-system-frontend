...
const BarGraphPerProgram: React.FC<BarGraphPerProgramProps> = ({ programName }) => {
    ...
    const fetchExhibitors = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          console.error(`Failed to fetch exhibitors for ${programName}`);
        }
      } catch (error) {
        console.error(`Error fetching exhibitors for ${programName}:`, error);
      }
    };
  ...
  // 1. Filter by the specific Program Name
  ...
  // 4. Map the categories and series
  ...
  const options: ApexOptions = {
    colors: ['#3C50E0'], // You can make this dynamic if you want different colors per program
    chart: {
    ...
    legend: { show: false }, // Hidden since there is only 1 series
    fill: { opacity: 1 },
  };
