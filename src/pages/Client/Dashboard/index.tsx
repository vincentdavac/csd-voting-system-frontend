...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const exhRes = await fetch(`${API_BASE_URL}/exhibitors/grouped-by-program`, {
        method: 'GET',
        headers: {
      ...
      const mappedSections: PROGRAM_SECTION[] = groupedData.map((group: any) => ({
        id: group.program.id,
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen dark:bg-gray-900 flex items-center justify-center py-6">
        <p className="text-xl text-[#071c4f] dark:text-white font-semibold italic">Loading Projects...</p>
      </div>
    );
  }
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {section.exhibitors.length > 0 ? (
              ...
              <div className="col-span-full text-center py-4">
                <p className="text-gray-500 italic">No exhibitors for this program.</p>
              </div>
            )}
          </div>
        </div>
